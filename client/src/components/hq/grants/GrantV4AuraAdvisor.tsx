...
export const GrantV4AuraAdvisor: React.FC = () => {
  const [question, setQuestion] = useState("");

  const questions = useQuery({
    queryKey: ["grant-v4-aura-questions"],
    queryFn: grantsApi.v4AuraQuestions,
    staleTime: 300_000,
  });

  const briefing = useQuery({
    queryKey: ["grant-v4-aura-briefing"],
    queryFn: () => grantsApi.v4AuraAdvisor(),
    staleTime: 120_000,
  });

  const ask = useMutation({
    mutationFn: (q: string) => grantsApi.v4AuraAdvisor(q),
  });
  ...
  return (
    <div className="hq-fade-in">
      <HqPanel title="AURA Executive Advisor" subtitle="Grant operations intelligence for IFCDC leadership">
        {briefing.isLoading ? (
          <HqLoading />
        ) : (
          <>
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem", flexWrap: "wrap" }}>
              <input
                ...
                placeholder="Ask about priorities, gaps, deadlines, pending awards, or 12-month forecast…"
                value={question}
            ...
            {data?.offline && <StatusBadge label="Offline advisor briefing" variant="warning" />}

            {data?.forecast && (
              <div style={{ fontSize: "0.82rem", color: "var(--hq-gold)", marginBottom: "0.5rem" }}>
                12-month projection: {fmt(data.forecast.total12MonthProjection)}
              </div>
            )}
            ...
            <pre style={{ whiteSpace: "pre-wrap", fontFamily: "inherit", fontSize: "0.85rem", lineHeight: 1.65, color: "var(--hq-text-muted)", margin: "0.75rem 0" }}>
              {data?.insight ?? "Ask AURA an executive funding operations question."}
            </pre>

            {(data?.topPriorities ?? []).length > 0 && (
              <div style={{ marginTop: "1rem" }}>
                <h4 style={{ fontSize: "0.85rem", color: "var(--hq-gold)", marginBottom: "0.5rem" }}>Recommended Priorities</h4>
                  ...
                  {data!.topPriorities.slice(0, 5).map((g) => (
                    <li key={String(g.id)} className="hq-activity-item">
                      <div className="hq-activity-content">
                        ...
                        <div className="hq-activity-detail">{String(g.funder)}</div>
                      </div>
                    </li>
                  ))}
                </ul>
